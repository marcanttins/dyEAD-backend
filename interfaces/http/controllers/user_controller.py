# backend/interface/http/controllers/user_controller.py
...
def list_users():
    ...
    repo = UserRepositoryImpl()
    try:
        # Retorna entidades de domínio, não dicts
        items, page_num, pages, total = repo.paginate(page, per_page)
    except APIError as e:
    ...
    # Monta payload com as entidades puras
    payload = {
        'items': items,
        'page' : page_num,
        'pages': pages,
    ...
    # Deixa o PaginationSchema serializar everything, inclusive nested users
...
def update_user(user_id: int):
    try:
        data = _user_req_schema.load(request.get_json(), partial=True)
    except ValidationError as err:
        return jsonify({'message': 'Erro de validação', 'errors': err.messages}), 400
